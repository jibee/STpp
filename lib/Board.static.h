...
HBridgeST<GeneralPurposeTimer<uint32_t, 4>> HBridge0(Prop0A, Prop0B, Prop0_PWM, Tim2, 3);

Gpio Enc0A(GpioE[9]);
Gpio Enc0B(GpioE[11]);
IncrementalEncoder16 Encoder0(Enc0A, Enc0B, Tim1, 0, false);

Gpio Prop1_PWM(GpioB[11]);
Gpio Prop1A(GpioB[14]);
Gpio Prop1B(GpioB[12]);
HBridgeST<GeneralPurposeTimer<uint32_t, 4>> HBridge1(Prop1B, Prop1A, Prop1_PWM, Tim2, 4);

Gpio Enc1A(GpioB[4]);
Gpio Enc1B(GpioB[5]);
IncrementalEncoder16 Encoder1(Enc1A, Enc1B, Tim3, 0, true);

Gpio Prop2_PWM(GpioC[7]);
Gpio Prop2A(GpioC[8]);
Gpio Prop2B(GpioC[12]);
HBridgeST<AdvancedControlTimer> Hbridge2(Prop2A, Prop2B, Prop2_PWM, Tim8, 2);

Gpio Prop3_PWM(GpioC[6]);
Gpio Prop3A(GpioC[9]);
Gpio Prop3B(GpioC[13]);
HBridgeST<AdvancedControlTimer> Hbridge3(Prop3A, Prop3B, Prop3_PWM, Tim8, 1);
...
DmaStream ZigbeeUartDma(DmaStream::DMAController1, DmaStream::S4, DmaStream::C4);
...
Pwm<GeneralPurposeTimer<uint16_t, 4>> LedG(g_LedG, Tim4, 1);

Gpio g_LedO(GpioD[13]);
Pwm<GeneralPurposeTimer<uint16_t, 4>> LedO(g_LedO, Tim4, 2);

Gpio g_LedR(GpioD[14]);
Pwm<GeneralPurposeTimer<uint16_t, 4>> LedR(g_LedR, Tim4, 3);

Gpio g_LedB(GpioD[15]);
Pwm<GeneralPurposeTimer<uint16_t, 4>> LedB(g_LedB, Tim4, 4);
...
Gpio Ax12Rx(GpioA[3]);
Gpio Ax12En(GpioA[1]);
DmaStream Ax12UartDma(DmaStream::DMAController1, DmaStream::S6, DmaStream::C4);
...
DmaStream ExternalUartDma(DmaStream::DMAController1, DmaStream::S3, DmaStream::C4);
